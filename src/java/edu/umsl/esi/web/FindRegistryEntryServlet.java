...
public class FindRegistryEntryServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (input != null && input.length() > 0) {
				...
				reader.close();
                                ...
                                String myid = entryObject.getString("id");
				...
				String myinheri = entryObject.getString("useInheritance");
                                //String myConf = entryObject.getString("confidential");
                                //confidential
				if (myscope != null && myscope.length() > 0 && myname != null && myname.length() == 0) {
					List<RegistryEntry> regentrlist = regentrdao.listRegistryEntriesByScope(myscope);
						...
						regentrlist = regentrdao.listRegistryEntriesByScope(myscope);
                                        ...
                                        /*if (myConf.equals("true")) {
						regentrlist = regentrdao.listRegistryEntriesByScopeConf(myscope);
					} else {
						regentrlist = regentrdao.listRegistryEntriesByScope(myscope);
					}*/
					...
					if (regentrlist.isEmpty()) {
						...
						// System.out.println("jsonarr: = " + jsonarr);
                                ...
                                else if(myid != null && myid.length() > 0) {
                                    int hasid = Integer.parseInt(entryObject.getString("id"));

					List<RegistryEntry> regentrlist = regentrdao.listRegistryEntriesById(hasid);
					...
					List<RegistryEntry> regentrlist = regentrdao.listRegistryEntriesByName(myname);
		...
		if(resultmsg == "No search result is found.")
			 jsonObject = Json.createObjectBuilder().add("resultmsg", resultmsg).build();
		else 
			 jsonObject = Json.createObjectBuilder().add("resultmsg", resultmsg).add("results", jsonarr).build();
